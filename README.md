...
In this code, the rotation_vector() function is changed to use the
cubic spline interpolation method. The ω and ω_̇ variables are then
computed using the cubic spline interpolation method. The ω variable
is then solved for using the least squares method. The t_poly variable
is fit to the time matrix using the cubic spline interpolation method.
The C variable is then interpolated using the cubic spline
interpolation method.

The if __name__ == '__main__': block is used to test the code. In this
block, the function is called with the same arguments as before. The
function then prints the interpolated attitude matrices.
